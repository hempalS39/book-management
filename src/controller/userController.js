...
const createUser = async function (req , res) {
    try {
        ...
        if (validPassword(password)===false) {return res.status(400).send({status:false, message: "password shoud be 8 to 15 characters which contain at least one numeric digit, one uppercase and one lowercase letter and one special character"})}
...
let loginUser=async function(req,res){
    try {
        ...
        const password=data.password
            ...
            if(!findEmail)return res.status(401).send({status:false,msg:"User not found"})
